	...
	<div class="container">
		...
		<p>
			<a class="btn btn-primary" data-toggle="collapse"
				...
				我要換/共${fn:length(MemberSupport)}筆 </a>
	...
	查無資料<br>
							...
							<c:forEach var='s' varStatus='vs' items='${MemberSupport }'>
								...
								<tr>
									...
									<c:choose>
										...
										<c:when
											test="${s.status == 1 || s.status ==2 ||s.status ==3 || s.status ==4}">
    							...
    							已換出，與 ${s.changehistorybean.partyB.sAccount } 交換
											...
											<c:when test="${s.status == 2}">
												<a class="btn btn-primary btn-sm"
													...
													href='<c:url value="showApplyFor?no=${s.changehistorybean.no }"/>'>待審核</a>
		...
		<p>
			<a class="btn btn-primary" data-toggle="collapse"
				...
				aria-controls="collapseExample1"> 我要徵
				/共${fn:length(MemberDemand)}筆 </a>
		</p>
		...
		<p>
			<a class="btn btn-primary" data-toggle="collapse"
				...
				aria-controls="collapseExample2"> 我的遊戲庫
				/共${fn:length(MemberGames)}筆 </a>
		</p>
									...
									<tr>
										...
										<c:choose>
											<c:when test="${g.status==2 }">
												<td>待換中(申請交換 <span style="color: green">${g.changehistorybean.supportgamebean.gamename }</span>
													等待 <span style="color: green">${g.changehistorybean.partyA.sAccount }</span>
													的同意)
												</td>
											</c:when>
											<c:when test="${g.supportgamebean==null }">
												<td><a
													href="<c:url value="/exchange/myGameToSupportGame?no=${g.no }"/>">交換去</a>
												</td>
											</c:when>
											<c:otherwise>
												<td>已在交換清單中</td>
		...
		<a class="btn btn-primary" style="background-color: red;"
			href="<c:url value="/exchange/Index"/>">返回主頁</a>
	</div>
