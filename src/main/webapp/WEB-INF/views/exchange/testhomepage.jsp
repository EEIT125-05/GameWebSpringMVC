...
<body>
	<form method='get' action='<c:url value="/exchange/preparehomepage"/>'>
		...
		<input type="submit"> <label><input type="radio"
			name="search" value="gamename" checked>遊戲名稱</label> <label><input
			type="radio" name="search" value="area">地區</label> 
		<input type="hidden" id="search" value="${search}"> <!-- 藏參數 -->	
		...
		<script>
			window.onload = function() {
				...
				function changepage(){
					...
					console.log(i)
					xhr.open('GET',
							'<c:url value="/exchange/changepage" />'+'?page='+ i + '&search=' + search, true);
					xhr.send();
					xhr.onreadystatechange = function() {
						...
						if (xhr.readyState === 4 && xhr.status === 200) {
							...
							+"<li>遊戲名稱:"+g.list[i].gamename+"</li>"
							+"<li>遊戲數量:"+g.list[i].qty+"</li>"
							+"<li>遊戲所在地:"+g.list[i].gamelocation+"</li>"
							+"<li>運送方式:"+g.list[i].delivery+"</li>"
							+"<li>主機平台:"+g.list[i].console+"</li>"
							+"<li>玩家名稱:"+g.list[i].gamer+"</li>"
							+"<li>是否含特點"+g.list[i].dlc+"</li>"
							+"<li>備註:"+g.list[i].remark+"</li>"
							+"</ul></div>"
							}
