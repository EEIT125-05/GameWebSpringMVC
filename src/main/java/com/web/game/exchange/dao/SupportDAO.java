...
public class SupportDAO {
	...
	public List<GameBean> changePage(int page){
		...
		list = (List<GameBean>) session.createQuery(queryAll).
	...
	public List<GameBean> changePageByParam(int page, String search,String param){
		...
		Session session = factory.getCurrentSession();
		String hql=null;
		String queryGamename = 
				"FROM GameBean WHERE status = 0 AND gamename like '%"+ param +"%'";
		String queryArea = 
				"FROM GameBean WHERE status = 0 AND gamelocation like '%"+ param +"%'";
		...
		list = (List<GameBean>) session.createQuery(hql).
